<template>
  ...
  >
    ...
    <button
      v-if="!GStore.currentEmail && $route.name != 'login'"
      @click="$router.push('login')"
      class="bg-white text-gray-800 active:bg-gray-100 text-xs font-bold uppercase px-4 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none lg:mr-1 lg:mb-0 m-3"
    >
      Login
    </button>
    <div
    ...
    >
      <p class="text-white text-xs font-bold uppercase text-rose-700">
        Email: {{ GStore.currentEmail && GStore.currentEmail.email }}
      </p>
      <button
        @click="handleLogout"
